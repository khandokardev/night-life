import { PRIVACY } from '@/constants/legalContent';
  ...
  return (
    <View style={[styles.root, { backgroundColor: tc.bg }]}>
      <View style={[styles.header, { paddingTop: insets.top + WEB_TOP, backgroundColor: tc.headerBg, borderBottomColor: tc.border }]}>
        ...
        <Text style={[styles.title, { color: tc.text, fontFamily: 'Poppins_700Bold' }]}>{PRIVACY.title}</Text>
        <View style={{ width: 36 }} />
      </View>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: 16, paddingBottom: insets.bottom + 30 }}>
        <Text style={[styles.date, { color: tc.text3, fontFamily: 'Inter_400Regular' }]}>{PRIVACY.date}</Text>
        <View style={[styles.popiaBanner, { backgroundColor: tc.accent + '08', borderColor: tc.accent + '28' }]}>
          <Feather name="shield" size={16} color={tc.accent} />
          <Text style={[styles.popia, { color: tc.text2, fontFamily: 'Inter_400Regular' }]}>
            SA Plug processes all personal data in accordance with the{' '}
            <Text style={{ color: tc.accent, fontFamily: 'Poppins_600SemiBold' }}>Protection of Personal Information Act (POPIA)</Text>
            {' '}of South Africa.
          </Text>
        </View>
        <Text style={[styles.intro, { color: tc.text2, fontFamily: 'Inter_400Regular' }]}>{PRIVACY.intro}</Text>
...
const styles = StyleSheet.create({
  ...
  popiaBanner: { flexDirection: 'row', gap: 10, borderRadius: 12, padding: 12, borderWidth: 1, marginBottom: 14, alignItems: 'flex-start' },
  popia: { fontSize: 12, lineHeight: 18, flex: 1 },
